...
class retina_module(nn.Module):
    def __init__(self,inp,features,inp_sz,size=8,lstm_features=64 ,device="cpu"):
        ...
        self.lstm_1 =  nn.LSTMCell(lstm_features,lstm_features, bias=True)
        self.position_predictor = nn.Linear(lstm_features,2)
        self.set = 0
        self.device=device
    ...
    def forward(self,x):
        ...
        for image in x.split(1,dim=0):
            ...
            state_H2 = torch.zeros((image.size(2),  self.lstm_F),device=device)
            state_C2 = torch.zeros((image.size(2) , self.lstm_F),device=device)
            features_retina = []
            step_IND = 0
